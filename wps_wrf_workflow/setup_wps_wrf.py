...
def _has_glob(path, pattern):
    if not path.exists():
        return False
    return any(path.glob(pattern))


def _has_any(path, patterns):
    if not path.exists():
        return False
...
def _count_glob(path, pattern):
    if not path.exists():
        return 0
    return sum(1 for _ in path.glob(pattern))


def _real_complete(run_dir):
    return _has_glob(run_dir, 'wrfinput_d0*') and _has_glob(run_dir, 'wrfbdy_d0*')
...
def parse_args():
    yaml_config_help = {
     ...
     'auto_stages': 'flag to auto-select missing workflow stages based on existing outputs (default: False)',
     'wrfout_threshold': 'minimum wrfout file count to treat WRF as complete when auto_stages is enabled (default: 12)',
    ...
    parser.add_argument('--auto-stages', dest='auto_stages', action='store_true',
                        help='Auto-select missing workflow stages based on existing outputs.')
    parser.add_argument('--no-auto-stages', dest='auto_stages', action='store_false',
                        help='Disable auto stage selection and respect do_* flags in YAML.')
    parser.add_argument('--wrfout-threshold', type=int, default=None,
                        help='Override wrfout threshold used when auto-stages is enabled.')
    parser.set_defaults(auto_stages=None)
    ...
    params.setdefault('auto_stages', False)
    params.setdefault('wrfout_threshold', 12)
    ...
    if args.auto_stages is not None:
        params['auto_stages'] = args.auto_stages
    if args.wrfout_threshold is not None:
        params['wrfout_threshold'] = args.wrfout_threshold
...
def main(cycle_dt_str_beg, cycle_dt_str_end, cycle_int_h, sim_hrs, icbc_fc_dt, exp_name, exp_wrf_only, realtime, archive, hostname,
         ...
         upp_working_dir, upp_yaml, upp_domains, auto_stages, wrfout_threshold,
    ...
    for cc in range(n_cycles):
        ...
        if auto_stages:
            ...
            wrf_done = wrfout_count >= wrfout_threshold
            real_done = _real_complete(wrf_run_dir)
            metgrid_done = _has_glob(metgrid_dir, 'met_em.d0*')
            geogrid_done = _has_glob(geo_run_dir, 'geo_em.d0*')
            ungrib_done = _has_any(ungrib_dir, UNGRIB_PATTERNS)

            log.info(
                f'Auto-stage status for {cycle_str}: '
                f'wrfout={wrfout_count} (threshold={wrfout_threshold}), '
                f'real={real_done}, metgrid={metgrid_done}, '
                f'geogrid={geogrid_done}, ungrib={ungrib_done}'
            )
            ...
            if wrf_done:
                ...
                do_real = False
                do_wrf = False
                log.info('Auto-stage: WRF outputs already present; skipping all WPS/WRF stages.')
            elif real_done:
                get_icbc = False
                do_geogrid = False
                do_ungrib = False
                ...
                do_real = False
                do_wrf = True
                log.info('Auto-stage: real outputs present; running WRF only.')
            elif metgrid_done:
                get_icbc = False
                do_geogrid = False
                do_ungrib = False
                ...
                do_real = True
                do_wrf = True
                log.info('Auto-stage: metgrid outputs present; running real + WRF.')
            else:
                do_geogrid = not geogrid_done
                do_ungrib = not ungrib_done
                do_metgrid = True
                do_real = True
                do_wrf = True
                if not do_ungrib:
                    get_icbc = False
                log.info(
                    f'Auto-stage: metgrid missing; '
                    f'geogrid={do_geogrid}, ungrib={do_ungrib}, '
                    f'metgrid/real/wrf enabled.'
                )
        if auto_stages:
            stage_plan = []
            if get_icbc:
                stage_plan.append('get_icbc')
            if do_geogrid:
                stage_plan.append('geogrid')
            if do_ungrib:
                stage_plan.append('ungrib')
            if do_avg_tsfc:
                stage_plan.append('avg_tsfc')
            if do_metgrid:
                stage_plan.append('metgrid')
            if do_real:
                stage_plan.append('real')
            if do_wrf:
                stage_plan.append('wrf')
            if stage_plan:
                log.info(f'Auto-stage plan for {cycle_str}: {", ".join(stage_plan)}')
            else:
                log.info(f'Auto-stage plan for {cycle_str}: none')
